...
func main() {
	if len(os.Args) != 4 {
                fmt.Fprintf(os.Stderr, "usage: %s <control-domain> <min> <max>\n", os.Args[0])
                os.Exit(1)
        }
        ...
        // 2) Min domain
        min64, err := strconv.ParseUint(os.Args[2], 10, 32)
        if err != nil {
                fmt.Fprintf(os.Stderr, "invalid min value: %v\n", err)
                os.Exit(1)
        }
        min := uint32(min64)

        // 3) Max domain
        max64, err := strconv.ParseUint(os.Args[3], 10, 32)
        if err != nil {
        ...
        max := uint32(max64)
        if min > max {
                fmt.Fprintf(os.Stderr, "min cannot be greater than max\n")
                os.Exit(1)
        }
        ...
        // SCAN DOMAINS
        // **********************************************************************************************************************
        for domain := min; domain <= max; domain++ {
            fmt.Printf("\n--- Domain %d | 0x%x ---\n", domain,domain)
