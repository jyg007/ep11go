...
func clearmek(target ep11.Target_t, domain uint32, keyBytes []byte) (  error)  {
 	_  , err := ep11.AdminCommand(target,domain, C.XCP_ADM_CLEAR_WK,nil,[][]byte{keyBytes})        
        if err != nil {    
            return err
        } 
	return nil
...
func genrandommek(target ep11.Target_t, domain uint32, keyBytes []byte) ( []byte, error)  {
